class Player extends Phaser.GameObjects.Container {
    constructor(scene, x, y, texture) {
        ...
        // --- Stat Calculation ---
        // To make our item stats work, we define the player's BASE stats.
        // These are the stats the player has with no items equipped.
        const baseStats = {
            health: 100,
            stamina: 100,
            staminaRegen: 1,
            healthRegen: 0 // Player has no base health regen.
        };

        // We get the item database and then calculate the total bonus stats from equipped gear.
        const itemDb = scene.cache.json.get('itemDatabase');
        const bonusStats = GameDataManager.calculateTotalStats(itemDb);

        // We now calculate the player's FINAL stats for this level.
        // The '|| 0' is a safety check. If a stat isn't provided by gear, it defaults to 0.
        ...
        // The player starts the level with full health and stamina.
    ...
    update() {
        ...
        this.handleStamina();
        this.handleHealthRegen(); // NEW: Call the health regen function every frame.
        this.updateShieldVisual();
    }

    /**
     * This new function handles the health regeneration mechanic.
     * It works just like stamina regen.
     */
    handleHealthRegen() {
        // We only regenerate health if the player has a regen rate and is not at max health.
        if (this.healthRegenRate > 0 && this.health < this.maxHealth) {
            ...
            // Ensure health doesn't go over the maximum.
